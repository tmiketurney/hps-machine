 ...
 * FILE : pstamper.c
 *
 * DESC : Part Stamper process entry point
 *
 ...
 *
 *  Revision 1.5  2003/10/07 22:50:48  tmike
...
static char *cvsid = "$Id: pstamper.c,v 1.9 2003/10/10 20:57:09 tmike Exp $";
...
void pstamper_init()
{
    ...
    Log(LOG_ERROR,"[%s].Part Stamper process unable to complete initialization.",
...
#ifdef LOG_ENABLE
    Log(LOG_INIT,"[%s].EXITING pstamper_init().",Log_proc_list[this_proc]);
#endif

}

///////////////////////////////////////////////
static void h_ps_stamp_part( GENERIC_MSG * gm )
{
    int io_value;

#ifdef LOG_ENABLE
    if (lfptr->pstamper_flag)
    {
        Log(LOG_TRACE,"[%s].ENTERING h_ps_stamp_part( gm[%x] ).",
    ...
    io_value = turn_on( cylCsolenoid );

    ps_state++;

    io_value = wait_until( cylCextended, 1 );

    ps_state++;

    io_value = turn_off( cylCsolenoid );

    ps_state++;

    io_value = wait_until( cylCretracted, 1 );

#ifdef LOG_ENABLE
    if (lfptr->pstamper_flag)
    {
        Log(LOG_TRACE,"[%s].EXITING h_ps_stamp_part( gm[%x] ).",
                Log_proc_list[this_proc], gm);
    }
#endif
...
/////////////////////////////////////////
void h_ps_return_state( GENERIC_MSG *gm )
{
#ifdef LOG_ENABLE
    if (lfptr->pstamper_flag)
    {
...
/////////////////////////////////////////
void h_ps_reset( GENERIC_MSG *gm )
{
#ifdef LOG_ENABLE
    if (lfptr->pstamper_flag)
    {
...
#ifdef LOG_ENABLE
    Log(LOG_TRACE,"**PROCESS CREATION**[%s][%d].pstamper(this_proc[%d]).",
        __FILE__, __LINE__, PROC_pstamper);
#endif

    // initialize Part Stamper process
    //////////////////////////////////
    pstamper_init();
